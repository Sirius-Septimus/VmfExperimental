...
char* StorageEntry::allocateBuffer(int handle, int size)
{
    ...
    // Allow zero-length logical buffers so mutators can return exact raw byte output.
    if(size < 0)
    {
    ...
    const size_t allocSize{size == 0 ? 1u : static_cast<size_t>(size)};
    ...
    if(typeIsTmpBuffer)
    {
        ...
        if(UNALLOCATED_BUFFER == tmpBufferSizes[key])
        {
            ...
            tmpBufferValues[key] = (char*) malloc(allocSize);
    ...
    else
    {
        ...
        if(UNALLOCATED_BUFFER == bufferSizes[key])
        {
            ...
            bufferValues[key] = (char*) malloc(allocSize);
